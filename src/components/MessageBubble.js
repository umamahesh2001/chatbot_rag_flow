...
/* Racing bolt logo for assistant */
      ...
      <div className={`max-w-[75%] ${isUser ? "items-end" : "items-start"}`}>
        <div
          className={`px-4 py-3 rounded-2xl text-sm leading-relaxed
            ${isUser
              ? "bg-gradient-to-br from-flash/12 to-turbo/8 border border-flash/12 text-ghost speed-shadow"
              : "bg-obsidian/80 border border-white/[0.04] text-silver"
            }
        ...
        >
          ...
          {message.imageUrl && (
            <div className="mb-3">
              <img src={message.imageUrl} alt={message.content}
                className="rounded-xl max-w-full border border-flash/10 speed-shadow" loading="lazy" />
              <a href={message.imageUrl} target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 mt-2 text-[11px] text-bolt hover:text-spark transition-colors">
                <IconDownload size={10} /> Open full size
              </a>
            </div>
          )}

          {/* Content — rendered markdown for assistant, plain text for user */}
          ...
          {hasError && isUser && (
            <div className="flex items-center gap-2 mt-2 pt-2 border-t border-danger/15">
              ...
              <button onClick={() => retryMessage(message.content)}
                className="ml-auto flex items-center gap-1 text-[11px] text-bolt hover:text-spark transition-colors">
                <IconRetry /> Retry
              </button>
            </div>
          )}
          ...
          {hasError && !isUser && (
            <div className="flex items-center gap-2 mt-2 pt-2 border-t border-danger/15">
              <IconAlert className="text-danger shrink-0" />
              <span className="text-[11px] text-danger">Provider error</span>
            </div>
          )}
        ...
        {!isUser && message.provider && (
          <span className="inline-flex items-center gap-1 mt-1.5 text-[10px] text-muted px-2 tracking-wider uppercase">
            <span className="w-1 h-1 rounded-full bg-flash/30" />
            {message.provider}
          </span>
        )}
      </div>
