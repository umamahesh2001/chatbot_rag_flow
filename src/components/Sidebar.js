...
export default function Sidebar() {
  const {
    conversations, activeConversationId, sidebarOpen, setSidebarOpen,
    fetchConversations, selectConversation, newConversation, deleteConversation,
    documents, fetchDocuments,
  } = useChatStore();

  useEffect(() => { fetchConversations(); fetchDocuments(); }, [fetchConversations, fetchDocuments]);
    ...
    <aside className="w-72 h-screen flex flex-col bg-abyss/90 backdrop-blur-2xl border-r border-flash/5 relative z-10">
      ...
      <div className="px-3 pb-2">
        <Link href="/documents"
          ...
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
            className="shrink-0 text-muted/40 group-hover:text-flash/60 group-hover:translate-x-0.5 transition-all">
      ...
      <div className="p-3 border-t border-flash/5">
        <div className="flex items-center justify-center gap-2">
          <div className="w-1 h-1 rounded-full bg-flash/30" />
          <p className="text-[9px] text-muted tracking-wider uppercase">Multi-Provider Engine</p>
          <div className="w-1 h-1 rounded-full bg-flash/30" />
        </div>
      </div>
    </aside>
